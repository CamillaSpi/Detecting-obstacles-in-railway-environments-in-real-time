...
def test_images(data, images, results, checkpoint, resultant_masks_path, threshold = 0.5):
    ...
    with open(data, mode='r') as csv_file:
        ...
        for i,row in enumerate(gt):
            if i!=0:
                gt_dict.update({row[0]: int(row[1])})
                gt_num += 1
    print(gt_num)
    print(gt_dict)
    ...
    with open(resultant_csv, 'w', newline='') as res_file:
        ...
        for image in tqdm(gt_dict.keys()):
            ...
            obstacle = model(img).to(device)
            #prob = nn.Softmax(dim=1)(obstacle)
            #preds = torch.argmax(prob,dim=1).unsqueeze(1).item()
            if threshold == 0.5:
                preds = obstacle.round()
            else:
                predizioni_booleane = obstacle > threshold
                preds = predizioni_booleane.int()
            #print("preds",preds)
            
            truths.append(gt_dict[image])
    ...
    print("stampa:", type(truths), type(obstacles))
    create_roc(truths, obstacles)
